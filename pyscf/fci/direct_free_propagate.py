...
# we may need to repeat the Lanczos steps. Therefore it is better to write a function that includes all the opeartions below:
    ...
    with lib.with_omp_threads(fci.threads):
        #e, c = lib.davidson(hop, ci0, precond, tol=fci.conv_tol, lindep=fci.lindep)
#       A, alpha, beta = fci.lanczos_tridiagonal(hop, ci0, tol=tol, lindep=lindep,
#                      max_cycle=max_cycle, max_space=max_space, nroots=nroots,
#                      max_memory=max_memory, verbose=log, 
#                      tol_residual=tol_residual, **kwargs)

        A, alpha, beta = lib.lanczos_tridiagonalize(aop=hop, x0=ci0[0], tol=tol, lindep=lindep,
                       max_cycle=max_cycle, max_space=max_space, nroots=nroots, orthogonalize=True,
                       max_memory=max_memory, verbose=log, **kwargs)

   # evaluate the vectors on a time grid: 
   # d (t=0) = A.C0   
   # d (t+\tau) = Te^{-i \lambda_p \tau) T d(0)
   # T H_p T+ = L_p     
   ...
   #get the transformation matrix and eigenvalues 
   #comment: replace max_cycle by the number of lanczos iterations required for tridiagonalization.
...
#    w, T = scipy.linalg.eigh(Ht) 
    ...
    def evolution_op(T, v, ti):
        expv = numpy.exp(-1j*ti*v) 
        vTdag = numpy.zeros((len(v), len(v)),complex)
        for rows in range(len(v)):
        #    vTdag[rows,:] = expv[rows]*T[rows, :].conj() 
            vTdag[rows,:] = expv[rows]*T[:, rows].conj() 
        #U = numpy.matmul(T.conj().transpose(), vTdag)
        U = numpy.matmul(T, vTdag)
        return U

    tmax = time_limit(T, w, 1.e-6, len_tdiag)

    print("value of tmax", tmax) 

#    grid = numpy.arange(mintime, maxtime, tmax) 

    grid_large = [] 

    grid = numpy.arange(mintime, maxtime, stepsize) 

#   density = numpy.zeros((len(grid),norb,norb))
    density = []
    d = numpy.zeros((len(grid),len_tdiag), complex)

#   U = evolution_op(T, w, stepsize) 
...
##  We will evaluate dipole correlation function as well.
    ...
    for count, t in enumerate(grid):
        ...
        if count == 0:
#          temp = numpy.einsum('ji,i->j', A, numpy.array(ci0)[0])  #this operation is expensive. but an out-of-core implementation is possible perhaps.
#          d[count,:] = numpy.einsum('ji,i->j', T, temp)  #this operation is expensive. but an out-of-core implementation is possible perhaps.

           d[count,:] = numpy.einsum('ji,i->j', A, ci0[0])  #this operation is expensive. but an out-of-core implementation is possible perhaps.
#           d[count,0] = 1.0+1j*0.0 
        else:
 #         U = evolution_op(T, w, stepsize) 
           U = scipy.linalg.expm(-(1j)*Ht*stepsize)

           d[count,:] = numpy.einsum('ji,i->j', U, d[count-1,:]) 

#          evaluate steps in between 
#          grid_small = numpy.arange(t+.1, t+tmax, .1)   
#          k_old = d[count-1,:]
#          for m, x in enumerate(grid_small):
#              k_new = numpy.einsum('ji,i->j', U, k_old)
#              fcivec_small = numpy.einsum('ij,j->i', A.transpose(), k_new) #this is again a very expensive operation. storage is also problematic  

#              densa, densb = make_rdm1s_complex(fcivec_small, norb, nelec, link_index)

#              density.append(densa + densb)

#              grid_large.append(x)

#              tot_energy  = energy(h1e, eri, fcivec_small.real, norb, nelec, link_index)
#              tot_energy += energy(h1e, eri, fcivec_small.imag, norb, nelec, link_index)
#              print("Total Energy", tot_energy)
#              k_old = k_new


        print("norm of d", numpy.linalg.norm(d[count,:])) 

   #expand d[t] back to the original CI basis:
        fcivec = numpy.einsum('ij,j->i', A.transpose(), d[count,:]) #this is again a very expensive operation. storage is also problematic  
   ...
   #check for the values of d  

        if(True):  
#        if abs(d[count, len_tdiag-2].real**2-d[count, len_tdiag-2].imag**2) > 1.e-8:
           print("We have to refresh the lanczos process at", t, max(d[count, :].real))

           A, alpha, beta = lib.lanczos_tridiagonalize(aop=hop_complex, x0=fcivec, tol=tol, lindep=lindep,
                       max_cycle=max_cycle, max_space=max_space, nroots=nroots, orthogonalize=True,
                       max_memory=max_memory, verbose=log, **kwargs)

#           d[count,:] = numpy.einsum('ji,i->j', A, fcivec)  #this operation is expensive. but an out-of-core implementation is possible perhaps.

           d[count,:] = d[0,:]

           print("check norm of d0", numpy.linalg.norm(d[count,:])) 
...
#           w, T = scipy.linalg.eigh_tridiagonal(alpha[0:max_cycle,0].real, beta[0:max_cycle-1,0].real) 
...
#        return e+ecore, [ci.reshape(na,nb) for ci in c]
#    else:
    return min(w), grid_large, numpy.asarray(density)
