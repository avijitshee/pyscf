...
def lanczos_tridiagonalize(aop, x0, tol=1e-5, max_cycle=50, max_space=12,
              lindep=DAVIDSON_LINDEP, max_memory=MAX_MEMORY,
              dot=numpy.dot, callback=None, orthogonalize=False,
              nroots=1, lessio=False, pick=None, verbose=logger.WARN):
    r'''Davidson diagonalization method to solve  a c = e c.  Ref
    [1] E.R. Davidson, J. Comput. Phys. 17 (1), 87-94 (1975).
    [2] http://people.inf.ethz.ch/arbenz/ewp/Lnotes/chapter11.pdf
...
# following lines are to activate some library routine to tridiagonalize..
...
#    _incore = max_memory*1e6/b.nbytes > 14
    _incore = max_memory*1e6/x1.nbytes > 14
    ...
    for cycle in range(max_cycle):
        ...
        xs.extend(x1)
#        ax.extend(axt) #no need to store sigma vectors
#        if callable(callback):
#            callback(cycle, xs, ax)

#        x1 = axt.copy()

#        alpha.extend(dot(x1.conj(),axt.transpose()))
        alpha.extend(dot(x1,axt.conj().transpose()))

        if (cycle > 0) :
             r  = axt
             r -= alpha[cycle]*x1 
             r -= beta[cycle-1]*xs[cycle-1] 
        else:
             r  = axt
             r -= alpha[cycle]*x1 
#extend beta
        temp = dot(r, r.conj().transpose())

        if abs(temp) > tol:
           beta.extend(numpy.sqrt(abs(temp)))            
        else:
           beta.extend(temp)

#extend xs. Here I am storing all the xs vectors, but this might be an overkill.
        x1 = r/beta[cycle]

#following is the orthogonalization step for the xs vectors. This can be a bottleneck
...
#           norm = abs(numpy.linalg.norm(x1.conj()))
        ...
        log.debug('krylov cycle %d  r = %g', cycle, beta[cycle-1])
